            ...
            when {
                outputIndex >= 0 -> {
                    ...
                    if (image != null && decoderInfo.size > 0) {
                        ...
                        if (processed !== original && !processed.isRecycled) {
                            processed.recycle()
                        }
...
private fun createProcessedFrame(
    ...
    if (anime !== source && !anime.isRecycled) {
        anime.recycle()
    }
...
private fun blendBitmaps(
    ...
    for (i in resultPixels.indices) {
        ...
        val r = (((op shr 16 and 0xff) * inverse) + ((ap shr 16 and 0xff) * strength))
            .roundToInt()
            .coerceIn(0, 255)

        val g = (((op shr 8 and 0xff) * inverse) + ((ap shr 8 and 0xff) * strength))
            .roundToInt()
            .coerceIn(0, 255)

        val b = (((op and 0xff) * inverse) + ((ap and 0xff) * strength))
            .roundToInt()
            .coerceIn(0, 255)
...
private fun findByteBufferEncoder(mime: String): MediaCodecInfo {
    val codecList = android.media.MediaCodecList(
    ...
    return codecList.codecInfos.firstOrNull { info ->
        ...
        info.getCapabilitiesForType(mime)
            .colorFormats
            .contains(MediaCodecInfo.CodecCapabilities.COLOR_FormatYUV420SemiPlanar)
...
private fun muxAudio(
    ...
    try {
        ...
        if (videoTrack < 0) {
            throw IllegalStateException("Processed video track is missing.")
        }
...
private fun calculateBitRate(
    ...
    val pixelsPerSecond = width.toLong() * height * frameRate
